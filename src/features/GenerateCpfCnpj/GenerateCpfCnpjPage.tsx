...
export default function GenerateCpfCnpjPage() {
    ...
    function checkChecked() {
        return settings.featureOpenOnLoad === Features.GENERATE_CPF_CNPJ ? true : false;
    }
    function generateHandler(type: string = 'cpf') {
        const value = type == 'cpf' ? generateCpf() : generateValidCNPJ()
    ...
    function changeOnLoad(e: React.ChangeEvent<HTMLInputElement>) {
        if (e.target.checked) {
            settingsStore.setState({ settings: { ...settings, featureOpenOnLoad: Features.GENERATE_CPF_CNPJ } })
        } else {
    ...
    return (
        <div>
            <div className='p-4 border-1 border-base-300 rounded-box flex w-full flex-col'>
                <div className='mb-2'>
                    <h3 className="text-lg font-bold mb-4 flex gap-2">Generate CPF/CNPJ</h3>
                        ...
                        <div className='flex gap-4'>
                            <div className="relative group cursor-pointer">
                                ...
                                <div
                                    ...
                                    <div className="space-y-2">
                                        <button className="btn btn-warning " onClick={() => generateHandler('cpf')}>
                                            <ImMagicWand size={20} />
                                            CPF</button>
                                    </div>
                                </div>
                            ...
                            <div className="relative group cursor-pointer">
                                ...
                                <div
                                    className="relative  bg-white ring-1 ring-gray-900/5 rounded-lg leading-none flex items-top justify-start space-x-6">
                                    <div className="space-y-2">
                                        <button className="btn btn-warning " onClick={() => generateHandler('cnpj')}>
                                            <ImMagicWand size={20} />
                                            CNPJ</button>
                                    </div>
                                </div>
